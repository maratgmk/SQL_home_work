...
-- arliner_id справа больше, чем t1.id слева и в общей таблице появилась дополнительная информация, которая отсутствует в обоих
-- как model_name, side_number ....?
...
select service_class, avg(price) as avarage_price
from tickets
...
select count(id) as departure_count, status
from trips
group by status
...
select count(departure_count)
from (select departure, count(id) as departure_count
from trips 
where status = 'Departed'
group by departure) t;  -- утеряно одно departure_count ?
